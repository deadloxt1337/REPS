...
namespace REPS::text
{
    ...
    bool scanner::try_get(const std::string& key, std::string& value) noexcept
    {
        if (auto it = m_data.find(key); it != m_data.end())
        {
            ...
            something.erase(std::remove(something.begin(), something.end(), '|'), something.end());
            value = something;
            return true;
        }
    ...
    bool scanner::try_get(const std::string& key, uint16_t& value) noexcept
    {
        if (auto it = m_data.find(key); it != m_data.end())
        {
            value = atoi(it->second.c_str());
    ...
    bool scanner::try_get(const std::string& key, uint32_t& value) noexcept
    {
        if (auto it = m_data.find(key); it != m_data.end())
        {
            value = atoi(it->second.c_str());
    ...
    bool scanner::try_get(const std::string& key, uint8_t& value) noexcept
    {
        if (auto it = m_data.find(key); it != m_data.end())
        {
            value = atoi(it->second.c_str());
    ...
    bool scanner::try_get(const std::string& key, bool& value) noexcept
    {
        if (auto it = m_data.find(key); it != m_data.end())
        {
            int xvalue = atoi(it->second.c_str());
